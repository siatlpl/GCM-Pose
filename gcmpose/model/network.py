...
from gcmpose.model.blocks import PositionGetter
from gcmpose.model.position_encoding import RoPE2D, PositionEncodingSine2D
from gcmpose.model.generalized_mean_pooling import GeM2D as GeMeanPool2d
from gcmpose.model.dino_layers import (MemEffSelfAttention,
                                MemEffCrossAttention,
                                MemEffEncoderLayer,
                                MemEffDecoderLayer)
from gcmpose.model.cmmda import CMMDA
...
class model_arch(nn.Module):
    def __init__(self, cfg=None):
        ...
        # CMMDA: cross-modal multi-scale deformable attention for 2D-3D matching
        cmmda_feat_dim = cfg.get('cmmda_feat_dim', 256)
        cmmda_num_heads = cfg.get('cmmda_num_heads', 8)
        cmmda_num_layers = cfg.get('cmmda_num_layers', 2)
        cmmda_num_scales = cfg.get('cmmda_num_scales', 3)
        cmmda_deform_points = cfg.get('cmmda_deform_points', 4)
        self.cmmda = CMMDA(
            dino_feat_dim=self.backbone_feat_dim,
            feat_dim=cmmda_feat_dim,
            num_heads=cmmda_num_heads,
            num_layers=cmmda_num_layers,
            num_scales=cmmda_num_scales,
            num_deform_points=cmmda_deform_points,
        )
        self.dual_softmax_temperature = cfg.get('dual_softmax_temperature', 0.1)
    ...
    def extract_2d3d_correspondences(self, dino_feat, point_cloud,
                                     return_reconstructions=False):
        """
        Extract 2D and 3D feature descriptors via CMMDA for matching.

        dino_feat:   (B, 768, 32, 32) from extract_DINOv2_feature()
        point_cloud: (B, N, 3)        SfM point cloud

        Returns dict from CMMDA with keys:
            'feat_2d': (B, 1024, 256)
            'feat_3d': (B, 512, 256)
            'feat_2d_scales', 'feat_3d_scales'
            'recon_2d', 'recon_3d' (if return_reconstructions=True)
        """
        return self.cmmda(dino_feat, point_cloud,
                          return_reconstructions=return_reconstructions)

    def compute_matching_confidence(self, feat_2d, feat_3d):
        """
        Dual-softmax operator (LoFTR-style) for 2D-3D matching confidence.

        feat_2d: (B, N2, C) L2-normalized 2D descriptors
        feat_3d: (B, N3, C) L2-normalized 3D descriptors

        Returns C3D: (B, N2, N3) matching confidence scores
        """
        feat_2d = F.normalize(feat_2d, dim=-1)
        feat_3d = F.normalize(feat_3d, dim=-1)
        S = torch.einsum('bnc,bmc->bnm', feat_2d, feat_3d) / self.dual_softmax_temperature
        C3D = S.softmax(dim=-1) * S.softmax(dim=-2)
        return C3D
